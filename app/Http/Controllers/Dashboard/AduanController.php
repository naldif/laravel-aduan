...
class AduanController extends Controller
{
    public function index(): Response
    {
        ...
        return Inertia::render('Admin/Aduan/AduanIndex', compact('aduans', 'filters'));
    ...
    public function store(StoreAduanRequest $request)
    {
        ...
        $slug = Str::slug($validated['title'], '-') . '-' . $uniqueId;
        $aduan = Aduan::create(array_merge($validated, [
            'image' => $image->hashName(),
            'slug' => $slug,
            'kecamatan' => $request->selectedDistrictName,
            'id_kecamatan' => $request->selectedDistrict,
            'title' => $request->title,
    ...
    {
        $request->validate([
            'status' => 'required|in:Belum Ditangani,Verifikasi,Diproses,Selesai',
